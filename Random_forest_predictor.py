...
class Random_forest_predictor:
    ...
    def __init__(self, train_frame, test_frame, verbose_model=1, num_of_trees=15000, maxdepth=8):
        self.test_frame = test_frame
        self.train_frame = train_frame
        self.num_of_trees = num_of_trees
        self.model = ensemble.RandomForestRegressor(n_estimators=num_of_trees, max_depth=maxdepth, max_features="auto",
                                                        verbose=verbose_model, bootstrap=False)
        self.inputs = []
        self.output = []
    ...
    def double_chain_testing(self, chain_steps, column_to_overwrite_1, column_to_overwrite_2, model_2_inputs,
        ...
        if filename_2:
            ...
            model_2 = ensemble.RandomForestRegressor(n_estimators=1000, max_depth=13, max_features="auto", verbose=0)
...
def k_fold_Xvalidation(filename, k, mape=False):
    ...
    for test_set_index in tqdm(range(len(percentages))):
        ...
        train_frame = sets[0]
        test_frame = sets[1]
        predictor = Random_forest_predictor(train_frame, test_frame, verbose_model=0)
...
if __name__ == "__main__":
    ...
    train_frame = sets[0]
    test_frame = sets[1]
    predictor = Random_forest_predictor(train_frame, test_frame, verbose_model=1, num_of_trees=1000, maxdepth=16)
    predictor.train_model(filename="", inputs=["Hour", "AvgT", "AvgP", "Month % 1", "Month % 2", "Month % 3",
                                               "Month % 4", "Month % 5", "Month % 6", "Month % 7", "Month % 8",
                                               "Month % 9", "Month % 10", "Month % 11", "Month % 12"],
                          output="AvgP_next_value_1")
